...
from codebase.constants import d2_surface_directory
...
class D2SurfaceShapeDistributionExtractor(D2BaseShapeDistributionExtractor):
    def __init__(self, database, **kwargs):
        super(D2SurfaceShapeDistributionExtractor, self).__init__(database, **kwargs)
    ...
    def extract_feature(self):
        seed(self.seed)
        counter = 0
        print_info_nn(" >>> Adding D2 surface shape distribution for database {0} ... ".format(self._database.name))
        ...
        for complex_name in self._database.complexes.keys():
            ...
            for protein in proteins:
                ...
                for i in range(len(protein.residues)):
                    protein.residues[i].add_feature(Features.D2_SURFACE_SHAPE_DISTRIBUTION, distributions[i, :])
        print_info("took {0} seconds.".format((datetime.now() - overall_time).seconds))

    def _get_directory(self):
        return d2_surface_directory